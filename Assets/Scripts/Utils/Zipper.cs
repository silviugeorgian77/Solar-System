...
public class Zipper
{
    ...
    public static byte[] CompressDirectory(string sInDir, ProgressDelegate progress = null)
    {
        string[] sFiles = Directory.GetFiles(sInDir, "*.*", SearchOption.AllDirectories);
        int iDirLen = sInDir[sInDir.Length - 1] == Path.DirectorySeparatorChar ? sInDir.Length : sInDir.Length + 1;
        ...
        using (GZipStream str = new GZipStream(compressedStream, CompressionMode.Compress))
            foreach (string sFilePath in sFiles)
            {
                string sRelativePath = sFilePath.Substring(iDirLen);
                if (progress != null)
    ...
    public static void DecompressToDirectory(byte[] byteArray, string sDir, ProgressDelegate progress = null)
    {
        MemoryStream compressedStream;
        using (compressedStream = new MemoryStream(byteArray))
